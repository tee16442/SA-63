...
// The CustomertypeFunc type is an adapter to allow the use of ordinary
// function as Customertype mutator.
type CustomertypeFunc func(context.Context, *ent.CustomertypeMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f CustomertypeFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.CustomertypeMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.CustomertypeMutation", m)
	}
	return f(ctx, mv)
}
