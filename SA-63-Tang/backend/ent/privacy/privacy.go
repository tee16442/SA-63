...
// The CustomertypeQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type CustomertypeQueryRuleFunc func(context.Context, *ent.CustomertypeQuery) error

// EvalQuery return f(ctx, q).
func (f CustomertypeQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.CustomertypeQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.CustomertypeQuery", q)
}

// The CustomertypeMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type CustomertypeMutationRuleFunc func(context.Context, *ent.CustomertypeMutation) error

// EvalMutation calls f(ctx, m).
func (f CustomertypeMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.CustomertypeMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.CustomertypeMutation", m)
}
