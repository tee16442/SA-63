...
// QueryCustomer2 chains the current query on the customer2 edge.
func (gq *GenderQuery) QueryCustomer2() *CustomerQuery {
...
//  WithCustomer2 tells the query-builder to eager-loads the nodes that are connected to
// the "customer2" edge. The optional arguments used to configure the query builder of the edge.
func (gq *GenderQuery) WithCustomer2(opts ...func(*CustomerQuery)) *GenderQuery {
	query := &CustomerQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
	gq.withCustomer2 = query
	return gq
}
...
func (gq *GenderQuery) sqlAll(ctx context.Context) ([]*Gender, error) {
	...
	if query := gq.withCustomer2; query != nil {
		...
		query.Where(predicate.Customer(func(s *sql.Selector) {
			s.Where(sql.InValues(gender.Customer2Column, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.gender
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "gender" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "gender" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Customer2 = append(node.Edges.Customer2, n)
		}
	}
