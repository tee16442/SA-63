...
// QueryCustomer4 chains the current query on the customer4 edge.
func (tq *TitleQuery) QueryCustomer4() *CustomerQuery {
...
//  WithCustomer4 tells the query-builder to eager-loads the nodes that are connected to
// the "customer4" edge. The optional arguments used to configure the query builder of the edge.
func (tq *TitleQuery) WithCustomer4(opts ...func(*CustomerQuery)) *TitleQuery {
	query := &CustomerQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withCustomer4 = query
	return tq
}
...
func (tq *TitleQuery) sqlAll(ctx context.Context) ([]*Title, error) {
	...
	if query := tq.withCustomer4; query != nil {
		...
		query.Where(predicate.Customer(func(s *sql.Selector) {
			s.Where(sql.InValues(title.Customer4Column, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.title
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "title" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "title" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Customer4 = append(node.Edges.Customer4, n)
		}
	}
