...
type CustomerQuery struct {
	...
	withCustomertype *CustomertypeQuery
...
// QueryCustomertype chains the current query on the customertype edge.
func (cq *CustomerQuery) QueryCustomertype() *CustomertypeQuery {
	query := &CustomertypeQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
//  WithCustomertype tells the query-builder to eager-loads the nodes that are connected to
// the "customertype" edge. The optional arguments used to configure the query builder of the edge.
func (cq *CustomerQuery) WithCustomertype(opts ...func(*CustomertypeQuery)) *CustomerQuery {
	query := &CustomertypeQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withCustomertype = query
	return cq
}
...
func (cq *CustomerQuery) sqlAll(ctx context.Context) ([]*Customer, error) {
	var (
		...
		loadedTypes = [3]bool{
			cq.withGender != nil,
			cq.withCustomertype != nil,
			cq.withTitle != nil,
		}
	)
	if cq.withGender != nil || cq.withCustomertype != nil || cq.withTitle != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withGender; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "gender" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := cq.withCustomertype; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Customer)
		for i := range nodes {
			if fk := nodes[i].customertype; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(customertype.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "customertype" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Customertype = n
			}
		}
	}
	...
	if query := cq.withTitle; query != nil {
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "title" returned %v`, n.ID)
			}
			for i := range nodes {
