...
func (tx *Tx) init() {
	tx.Adult = NewAdultClient(tx.config)
	tx.Books = NewBooksClient(tx.config)
	tx.Customer = NewCustomerClient(tx.config)
	tx.Kid = NewKidClient(tx.config)
	tx.Room = NewRoomClient(tx.config)
	tx.Roomamount = NewRoomamountClient(tx.config)
}
...
// applies a query, for example: Adult.QueryXXX(), the query will be executed
