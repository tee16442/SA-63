...
func (cq *CustomerQuery) QueryBooks() *BooksQuery {
...
func (cq *CustomerQuery) WithBooks(opts ...func(*BooksQuery)) *CustomerQuery {
	query := &BooksQuery{config: cq.config}
...
func (cq *CustomerQuery) sqlAll(ctx context.Context) ([]*Customer, error) {
	...
	if query := cq.withBooks; query != nil {
		...
		for _, n := range neighbors {
			fk := n.customer_books
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "customer_books" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "customer_books" returned %v for node %v`, *fk, n.ID)
			}
