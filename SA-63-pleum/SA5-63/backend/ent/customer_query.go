...
// QueryCheckout2 chains the current query on the checkout2 edge.
func (cq *CustomerQuery) QueryCheckout2() *CheckoutQuery {
...
//  WithCheckout2 tells the query-builder to eager-loads the nodes that are connected to
// the "checkout2" edge. The optional arguments used to configure the query builder of the edge.
func (cq *CustomerQuery) WithCheckout2(opts ...func(*CheckoutQuery)) *CustomerQuery {
	query := &CheckoutQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withCheckout2 = query
	return cq
}
...
func (cq *CustomerQuery) sqlAll(ctx context.Context) ([]*Customer, error) {
	...
	if query := cq.withCheckout2; query != nil {
		...
		query.Where(predicate.Checkout(func(s *sql.Selector) {
			s.Where(sql.InValues(customer.Checkout2Column, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.customer_checkout2
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "customer_checkout2" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "customer_checkout2" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Checkout2 = append(node.Edges.Checkout2, n)
		}
	}
