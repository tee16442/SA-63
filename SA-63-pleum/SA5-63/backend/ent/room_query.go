...
// QueryCheckout4 chains the current query on the checkout4 edge.
func (rq *RoomQuery) QueryCheckout4() *CheckoutQuery {
	query := &CheckoutQuery{config: rq.config}
...
//  WithCheckout4 tells the query-builder to eager-loads the nodes that are connected to
// the "checkout4" edge. The optional arguments used to configure the query builder of the edge.
func (rq *RoomQuery) WithCheckout4(opts ...func(*CheckoutQuery)) *RoomQuery {
	query := &CheckoutQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withCheckout4 = query
	return rq
}
...
func (rq *RoomQuery) sqlAll(ctx context.Context) ([]*Room, error) {
	...
	if query := rq.withCheckout4; query != nil {
		...
		query.Where(predicate.Checkout(func(s *sql.Selector) {
			s.Where(sql.InValues(room.Checkout4Column, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.room_checkout4
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "room_checkout4" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "room_checkout4" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Checkout4 = n
		}
	}
