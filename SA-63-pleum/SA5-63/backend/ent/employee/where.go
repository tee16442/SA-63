...
// HasCheckout3 applies the HasEdge predicate on the "checkout3" edge.
func HasCheckout3() predicate.Employee {
	return predicate.Employee(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(Checkout3Table, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, Checkout3Table, Checkout3Column),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasCheckout3With applies the HasEdge predicate on the "checkout3" edge with a given conditions (other predicates).
func HasCheckout3With(preds ...predicate.Checkout) predicate.Employee {
	return predicate.Employee(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(Checkout3InverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, Checkout3Table, Checkout3Column),
		)
