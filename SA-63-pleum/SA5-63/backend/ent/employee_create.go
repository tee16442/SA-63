...
func (ec *EmployeeCreate) AddCheckout3IDs(ids ...int) *EmployeeCreate {
	ec.mutation.AddCheckout3IDs(ids...)
	return ec
}

// AddCheckout3 adds the checkout3 edges to Checkout.
func (ec *EmployeeCreate) AddCheckout3(c ...*Checkout) *EmployeeCreate {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return ec.AddCheckout3IDs(ids...)
}
...
func (ec *EmployeeCreate) createSpec() (*Employee, *sqlgraph.CreateSpec) {
	...
	if nodes := ec.mutation.Checkout3IDs(); len(nodes) > 0 {
