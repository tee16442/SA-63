...
// QueryCheckout3 chains the current query on the checkout3 edge.
func (eq *EmployeeQuery) QueryCheckout3() *CheckoutQuery {
	query := &CheckoutQuery{config: eq.config}
...
//  WithCheckout3 tells the query-builder to eager-loads the nodes that are connected to
// the "checkout3" edge. The optional arguments used to configure the query builder of the edge.
func (eq *EmployeeQuery) WithCheckout3(opts ...func(*CheckoutQuery)) *EmployeeQuery {
	query := &CheckoutQuery{config: eq.config}
	for _, opt := range opts {
		opt(query)
	}
	eq.withCheckout3 = query
	return eq
}
...
func (eq *EmployeeQuery) sqlAll(ctx context.Context) ([]*Employee, error) {
	...
	if query := eq.withCheckout3; query != nil {
		...
		query.Where(predicate.Checkout(func(s *sql.Selector) {
			s.Where(sql.InValues(employee.Checkout3Column, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.employee_checkout3
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "employee_checkout3" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "employee_checkout3" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Checkout3 = append(node.Edges.Checkout3, n)
		}
	}
