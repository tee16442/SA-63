...
// The RoomstateFunc type is an adapter to allow the use of ordinary
// function as Roomstate mutator.
type RoomstateFunc func(context.Context, *ent.RoomstateMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f RoomstateFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.RoomstateMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.RoomstateMutation", m)
	}
	return f(ctx, mv)
}
