...
// QueryRoom2 chains the current query on the Room2 edge.
func (pq *PromotionQuery) QueryRoom2() *RoomQuery {
...
//  WithRoom2 tells the query-builder to eager-loads the nodes that are connected to
// the "Room2" edge. The optional arguments used to configure the query builder of the edge.
func (pq *PromotionQuery) WithRoom2(opts ...func(*RoomQuery)) *PromotionQuery {
	query := &RoomQuery{config: pq.config}
...
//		GroupBy(promotion.FieldPROMOTIONDETAIL).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Promotion.Query().
//		Select(promotion.FieldPROMOTIONDETAIL).
//		Scan(ctx, &v)
//
...
func (pq *PromotionQuery) sqlAll(ctx context.Context) ([]*Promotion, error) {
	...
	if query := pq.withRoom2; query != nil {
		...
		query.Where(predicate.Room(func(s *sql.Selector) {
			s.Where(sql.InValues(promotion.Room2Column, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.promotion_room2
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "promotion_room2" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "promotion_room2" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Room2 = append(node.Edges.Room2, n)
		}
	}
