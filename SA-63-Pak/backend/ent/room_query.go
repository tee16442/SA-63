...
func (rq *RoomQuery) QueryRoomtype() *RoomtypeQuery {
	query := &RoomtypeQuery{config: rq.config}
...
func (rq *RoomQuery) QueryPromotion() *PromotionQuery {
	query := &PromotionQuery{config: rq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryRoomstate chains the current query on the roomstate edge.
func (rq *RoomQuery) QueryRoomstate() *RoomstateQuery {
	query := &RoomstateQuery{config: rq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoomQuery) WithRoomtype(opts ...func(*RoomtypeQuery)) *RoomQuery {
	query := &RoomtypeQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withRoomtype = query
	return rq
}
...
func (rq *RoomQuery) WithPromotion(opts ...func(*PromotionQuery)) *RoomQuery {
	query := &PromotionQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withPromotion = query
	return rq
}

//  WithRoomstate tells the query-builder to eager-loads the nodes that are connected to
// the "roomstate" edge. The optional arguments used to configure the query builder of the edge.
func (rq *RoomQuery) WithRoomstate(opts ...func(*RoomstateQuery)) *RoomQuery {
	query := &RoomstateQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withRoomstate = query
	return rq
}
...
func (rq *RoomQuery) sqlAll(ctx context.Context) ([]*Room, error) {
	var (
		...
		loadedTypes = [3]bool{
			rq.withRoomtype != nil,
			rq.withPromotion != nil,
			rq.withRoomstate != nil,
		}
	)
	if rq.withRoomtype != nil || rq.withPromotion != nil || rq.withRoomstate != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := rq.withRoomtype; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Room)
		for i := range nodes {
			if fk := nodes[i].roomtype_room1; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "roomtype_room1" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := rq.withPromotion; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Room)
		for i := range nodes {
			if fk := nodes[i].promotion_room2; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "promotion_room2" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := rq.withRoomstate; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Room)
		for i := range nodes {
			if fk := nodes[i].roomstate_room3; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(roomstate.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "roomstate_room3" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Roomstate = n
			}
		}
	}
