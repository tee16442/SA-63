...
// The RoomstateQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type RoomstateQueryRuleFunc func(context.Context, *ent.RoomstateQuery) error

// EvalQuery return f(ctx, q).
func (f RoomstateQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.RoomstateQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.RoomstateQuery", q)
}

// The RoomstateMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type RoomstateMutationRuleFunc func(context.Context, *ent.RoomstateMutation) error

// EvalMutation calls f(ctx, m).
func (f RoomstateMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.RoomstateMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.RoomstateMutation", m)
}
