...
func (cq *CustomerQuery) QueryProblem() *ProblemQuery {
	query := &ProblemQuery{config: cq.config}
...
func (cq *CustomerQuery) WithProblem(opts ...func(*ProblemQuery)) *CustomerQuery {
	query := &ProblemQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withProblem = query
	return cq
}
...
func (cq *CustomerQuery) sqlAll(ctx context.Context) ([]*Customer, error) {
	...
	if query := cq.withProblem; query != nil {
		...
		query.Where(predicate.Problem(func(s *sql.Selector) {
			s.Where(sql.InValues(customer.ProblemColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.customer_problem
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "customer_problem" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "customer_problem" returned %v for node %v`, *fk, n.ID)
			}
