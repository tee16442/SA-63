...
// The ProblemtypeQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type ProblemtypeQueryRuleFunc func(context.Context, *ent.ProblemtypeQuery) error

// EvalQuery return f(ctx, q).
func (f ProblemtypeQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.ProblemtypeQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.ProblemtypeQuery", q)
}

// The ProblemtypeMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type ProblemtypeMutationRuleFunc func(context.Context, *ent.ProblemtypeMutation) error

// EvalMutation calls f(ctx, m).
func (f ProblemtypeMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.ProblemtypeMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.ProblemtypeMutation", m)
}
