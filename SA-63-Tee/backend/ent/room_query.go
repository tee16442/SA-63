...
func (rq *RoomQuery) QueryProblem() *ProblemQuery {
...
func (rq *RoomQuery) WithProblem(opts ...func(*ProblemQuery)) *RoomQuery {
	query := &ProblemQuery{config: rq.config}
...
func (rq *RoomQuery) sqlAll(ctx context.Context) ([]*Room, error) {
	...
	if query := rq.withProblem; query != nil {
		...
		query.Where(predicate.Problem(func(s *sql.Selector) {
			s.Where(sql.InValues(room.ProblemColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.room_problem
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "room_problem" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "room_problem" returned %v for node %v`, *fk, n.ID)
			}
